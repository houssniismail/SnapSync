...
import fetchDataThunk from '../redux/thinks/fetchDataThunk';
const Carts = () => {
    const dispatch = useDispatch();
    const { data, loading, error } = useSelector((state) => state.fetchDataReducer);
        ...
        <div className='grid md:grid-cols-2 py-4 '>
            {
                data && data.map((item, index) => (
                    <div key={index} className='m-2 shadow-xl rounded-b-2xl'>
                        ...
                        <div className='p-2'>
                            <p className=' text-slate-500'>{item.Tags}</p>
                        </div>
